  ...
  return (
    <div className="p-6 md:p-8 space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <header className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          <h1 className="text-[length:var(--font-size-page-title)] font-heading font-[--font-weight-page-title] tracking-tight dark:text-[color:var(--color-page-title-dark)] mb-1">Поэзия и Стихи</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filtered.map(poem => (
          <Link
            key={poem.id}
            to={`/poem/${poem.id}`}
            className="group relative block rounded-3xl overflow-hidden bg-[color:var(--color-poem-card-bg-dark)] border-[length:var(--border-width-poem-card)] border-[color:var(--color-poem-card-border-dark)] hover:border-[color:var(--color-poem-card-hover-border-dark)] transition-all shadow-[--poem-card-shadow-dark] hover:shadow-[--poem-card-shadow-hover-dark] hover:-translate-y-0.5"
          >
            {/* Cover image */}
            ...
            <div className="p-5 relative">
              ...
              <div className="flex flex-wrap gap-1.5">
                {poem.tags.slice(0, 3).map(tag => (
                  <span
                    key={tag}
                    className="text-[10px] uppercase tracking-wider font-semibold text-zinc-500 bg-zinc-950/80 border border-zinc-800/50 px-1.5 py-0.5 rounded-md"
                  >
                    #{tag}
                  </span>
                ))}
