...
function SourceDropdown({ sources, selected, onSelect }: SourceDropdownProps) {
  ...
  return (
    <div ref={ref}>
      <p className="text-[length:var(--font-size-caption)] font-[--font-weight-heading] text-zinc-500 uppercase tracking-[--letter-spacing-widest] mb-3 flex items-center gap-1.5">
...
export default function Clips() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
      {/* Header */}
      <header className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          <h1 className="text-[length:var(--font-size-page-title)] font-heading font-[--font-weight-page-title] tracking-tight dark:text-[color:var(--color-page-title-dark)] mb-1">Дорамы</h1>
          <p className="text-[length:var(--font-size-body)] text-text-muted-dark">Изучайте корейский через короткие сцены из ваших любимых дорам.</p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-[--font-weight-body] transition-all',
              showFilters
                ? 'bg-indigo-600 border-indigo-500 text-white'
                : 'bg-background-dark/80 border-border-dark/50 text-text-muted-dark hover:text-white hover:border-border-dark'
            )}
          >
      ...
      {showFilters && (
        <div className="bg-zinc-900/70 rounded-2xl border border-zinc-800/70 p-5 space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          {FILTER_GROUPS.map(group => (
            <div key={group.label}>
              <p className="text-[length:var(--font-size-micro)] font-[--font-weight-heading] text-text-muted-dark uppercase tracking-[--letter-spacing-widest] mb-3 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredClips.map(clip => (
            <Link
              key={clip.id}
              to={`/clip/${clip.id}`}
              className="group relative block rounded-3xl overflow-hidden bg-[color:var(--color-drama-card-bg-dark)] border-[length:var(--border-width-drama-card)] border-[color:var(--color-drama-card-border-dark)] hover:border-[color:var(--color-drama-card-hover-border-dark)] transition-all shadow-[--drama-card-shadow-dark] hover:shadow-[--drama-card-shadow-hover-dark] hover:-translate-y-0.5"
            >
              <div className="w-full aspect-video relative overflow-hidden bg-black shrink-0">
                ...
                <div className="absolute bottom-4 left-4 flex gap-2">
                  <span className={cn(
                    'px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-wider uppercase border',
                    LEVEL_COLORS[clip.level] || 'bg-indigo-500/80 text-white border-indigo-400/30'
                  )}>
                    {clip.level}
                  </span>
                  <span className="px-1.5 py-0.5 rounded-md bg-white/20 backdrop-blur-md text-white text-[9px] font-bold tracking-wider uppercase border border-white/20">
              ...
              <div className="p-5 relative">
                ...
                <h3 className="text-[length:var(--font-size-subheading)] font-[--font-weight-heading] text-text-main-dark mb-1.5 group-hover:text-indigo-300 transition-colors line-clamp-1">
                  {clip.title}
                </h3>
                <p className="text-[length:var(--font-size-small)] text-text-muted-dark mb-4 line-clamp-1">{clip.source}</p>
                ...
                <div className="flex flex-wrap gap-1">
                  {clip.tags.slice(0, 4).map(tag => (
                    <span
                      ...
                      className={cn(
                        'text-[9px] uppercase tracking-wider font-semibold px-1 py-0.5 rounded-md border cursor-pointer transition-colors',
                        activeTags.has(tag)
