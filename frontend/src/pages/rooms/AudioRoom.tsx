      ...
      <div className="flex-1 w-full max-w-4xl flex flex-col min-h-0">
        {activeTab === 'continue' ? (
          <div className="flex flex-col gap-6 h-full">
            ...
            {lastProgress ? (
              <div 
                className="bg-gradient-to-br from-[color:var(--color-audio-card-from)] via-[color:var(--color-audio-card-via)] to-[color:var(--color-audio-card-to)] rounded-3xl p-6 md:p-8 border border-[color:var(--color-audio-card-border)] shadow-2xl shadow-[color:var(--color-audio-card-shadow)] relative overflow-hidden group cursor-pointer hover:border-blue-400/40 transition-all duration-500 hover:shadow-blue-500/20"
                onClick={() => navigate(lastProgress.isLocal ? '/local-audio-player' : `/player/${lastProgress.id}`)}  // route unchanged
              >
                <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center">
                  <div className="media-thumbnail-base media-thumbnail-audio group-item">
                    <div className="media-thumbnail-decoration" />
                    <Music className="w-16 h-16 text-teal-500/10 absolute -rotate-12" />
                    <Music className="w-10 h-10 text-teal-400/40 relative z-10" />
                    <div className="absolute inset-0 bg-teal-500/0 group-hover:bg-teal-600/20 transition-colors backdrop-blur-[1px] opacity-0 group-hover:opacity-100 flex items-center justify-center z-20">
                  ...
                  <div className="flex-1 w-full text-center md:text-left flex flex-col justify-center">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-2 text-[color:var(--color-audio-card-accent)] font-bold text-xs uppercase tracking-widest">
                       Продолжить прослушивание
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-xs font-mono text-zinc-500 max-w-md mx-auto md:mx-0">
                       ...
                       <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div 
                             className="h-full bg-[color:var(--color-audio-card-accent)] shadow-[0_0_10px_rgba(96,165,250,0.5)] transition-all duration-300" 
                 ...
                 {history.length <= 1 ? (
                   ...
                   <div className="grid grid-cols-1 gap-3">
                     {history.slice(1).map(item => (
                       <div 
                         key={item.id} 
                         className="group flex items-center gap-4 p-3 pr-4 bg-gradient-to-r from-[color:var(--color-audio-card-history-bg)] to-slate-900/40 backdrop-blur-md rounded-2xl border border-blue-500/10 shadow-sm hover:shadow-md hover:border-blue-500/30 transition-all cursor-pointer"
                         onClick={() => navigate(item.isLocal ? '/local-audio-player' : `/player/${item.id}`)}
                       >
                         <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-teal-900/40 to-slate-900/40 flex-shrink-0 flex items-center justify-center relative overflow-hidden border border-white/5 shadow-inner">
                           <div className="media-thumbnail-decoration opacity-10" />
                           <Music className="w-6 h-6 text-teal-400/30 relative z-10" />
                         ...
                         <div className="flex-1 min-w-0">
                           ...
                           <div className="flex items-center gap-2 mt-1">
                             <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                               <div className="h-full bg-[color:var(--color-audio-card-accent)]/60" style={{ width: `${item.duration ? (item.timestamp / item.duration) * 100 : 0}%`}} />
                             </div>
                         ...
                         <ChevronRight className="w-5 h-5 text-zinc-700 group-hover:text-[color:var(--color-audio-card-accent)] group-hover:translate-x-1 transition-all flex-shrink-0" />
                       </div>
                     ))}
                   </div>
                 )}
                 ...
                 <div className="w-full aspect-video md:aspect-[21/9] rounded-[2rem] bg-teal-50 dark:bg-teal-900/10 border border-teal-100 dark:border-teal-500/20 flex flex-col items-center justify-center p-8 relative z-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full max-w-lg justify-center">
                       ...
                       >
                         ...
                         <span className="whitespace-nowrap">{subtitleFile ? `Текст: ${subtitleFile.name.slice(0, 14)}…` : 'Загрузить текст'}</span>
                       </button>
