      ...
      <div className="flex-1 w-full max-w-4xl flex flex-col min-h-0">
        {activeTab === 'continue' ? (
          <div className="flex flex-col gap-6 h-full">
            ...
            {lastProgress ? (
              <div 
                className="bg-gradient-to-br from-[color:var(--color-reader-card-from)] via-[color:var(--color-reader-card-via)] to-[color:var(--color-reader-card-to)] rounded-3xl p-6 md:p-8 border border-[color:var(--color-reader-card-border)] shadow-2xl shadow-[color:var(--color-reader-card-shadow)] relative overflow-hidden group cursor-pointer hover:border-blue-400/40 transition-all duration-500 hover:shadow-blue-500/20"
                onClick={() => navigate(lastProgress.isLocal ? '/poem/local' : `/poem/${lastProgress.id}`)}
              >
                <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center">
                  <div className="media-thumbnail-base media-thumbnail-reader group-item">
                    <div className="media-thumbnail-decoration" />
                    <BookOpen className="w-16 h-16 text-purple-500/10 absolute rotate-6" />
                    <BookOpen className="w-10 h-10 text-purple-400/40 relative z-10" />
                    <div className="absolute inset-0 bg-purple-500/0 group-hover:bg-purple-600/20 transition-colors backdrop-blur-[1px] opacity-0 group-hover:opacity-100 flex items-center justify-center z-20">
                  ...
                  <div className="flex-1 w-full text-center md:text-left flex flex-col justify-center">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-2 text-[color:var(--color-reader-card-accent)] font-bold text-xs uppercase tracking-widest">
                       Продолжить чтение
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-xs font-mono text-zinc-500 max-w-md mx-auto md:mx-0">
                       ...
                       <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div 
                             className="h-full bg-[color:var(--color-reader-card-accent)] shadow-[0_0_10px_rgba(96,165,250,0.5)] transition-all duration-300" 
                 ...
                 {history.length <= 1 ? (
                   ...
                   <div className="grid grid-cols-1 gap-3">
                     {history.slice(1).map(item => (
                       <div 
                         key={item.id} 
                         className="group flex items-center gap-4 p-3 pr-4 bg-gradient-to-r from-[color:var(--color-reader-card-history-bg)] to-slate-900/40 backdrop-blur-md rounded-2xl border border-blue-500/10 shadow-sm hover:shadow-md hover:border-blue-500/30 transition-all cursor-pointer"
                         onClick={() => navigate(item.isLocal ? '/poem/local' : `/poem/${item.id}`)}
                       >
                         <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-900/40 to-slate-900/40 flex-shrink-0 flex items-center justify-center relative overflow-hidden border border-white/5 shadow-inner">
                           <div className="media-thumbnail-decoration opacity-10" />
                           <FileText className="w-6 h-6 text-purple-400/30 relative z-10" />
                         ...
                         <div className="flex-1 min-w-0">
                           ...
                           <div className="flex items-center gap-2 mt-1">
                             <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                               <div className="h-full bg-[color:var(--color-reader-card-accent)]/60" style={{ width: `${item.duration ? (item.timestamp / item.duration) * 100 : 0}%`}} />
                             </div>
                         ...
                         <ChevronRight className="w-5 h-5 text-zinc-700 group-hover:text-[color:var(--color-reader-card-accent)] group-hover:translate-x-1 transition-all flex-shrink-0" />
                       </div>
                     ))}
                   </div>
                 )}
