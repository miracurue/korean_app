...
import { Play, Pause, Volume2, Sparkles, Loader2, RotateCcw } from 'lucide-react';
    ...
    content: [
      {
        ...
        words: [
          ...
          { text: '주기', baseForm: '주다', translation: 'Давать', grammar: 'Глагол + окончание 기' },
      ...
      },
      {
        ...
        words: [
          ...
          { text: '지나지', baseForm: '지나다', translation: 'Проходить', grammar: '' },
...
export default function PoemReader() {
  ...
  return (
    <div className="p-4 md:p-6 pb-0 w-full max-w-[1600px] mx-auto lg:h-[calc(100vh-24px)] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-4 shrink-0 flex items-center justify-between px-2">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          <Link to="/poems" className="p-2 -ml-2 text-zinc-500 hover:text-white transition-colors rounded-full hover:bg-white/5">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
          </Link>
          <div className="flex flex-col">
            <h1 className="text-lg font-bold text-white leading-tight">
              {activePoem.koreanTitle}{' '}
              <span className="text-sm text-zinc-500 font-normal ml-1">({activePoem.title})</span>
            </h1>
        ...
        <FontSizeControl fontSize={fontSize} min={12} max={48} step={2} onChange={setFontSize} />
      </div>
      ...
      >
        {/* Left column: audio player + poem text */}
          ...
          <div className="w-full shrink-0 bg-zinc-900/80 p-6 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
            ...
            <div className="relative z-10 w-24 h-24 md:w-32 md:h-32 rounded-2xl overflow-hidden shadow-xl border border-white/10 shrink-0 group">
              ...
              <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={togglePlay} className="w-12 h-12 rounded-full bg-brand-cyan text-white flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-all">
            ...
            <div className="flex-1 w-full space-y-4 relative z-10">
              <div>
                <h2 className="text-brand-cyan text-sm font-semibold uppercase tracking-wider mb-1">Оригинальное чтение</h2>
                <div className="flex items-center gap-4">
                  <button onClick={togglePlay} className="p-3 bg-brand-cyan hover:bg-brand-cyan/80 text-white rounded-full transition-all shadow-lg shadow-brand-cyan/20 active:scale-95">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="w-full h-1.5 bg-zinc-800 rounded-full cursor-pointer relative group/progress" onClick={handleProgressClick}>
                      <div className="absolute top-0 left-0 h-full bg-brand-cyan rounded-full transition-all duration-100" style={{ width: `${progress}%` }} />
                  ...
                  <div className="hidden md:flex items-center gap-2 group/vol">
                    <Volume2 size={16} className="text-zinc-500 group-hover/vol:text-brand-cyan transition-colors" />
                    <div className="w-16 h-1 bg-zinc-800 rounded-full cursor-pointer relative overflow-hidden">
                      <div className="absolute top-0 left-0 h-full bg-brand-cyan" style={{ width: `${volume * 100}%` }} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide p-6 lg:p-10 bg-zinc-950/20">
            <div className="max-w-3xl mx-auto space-y-12">
              {activePoem.content.map((line, idx) => (
                <div
                  key={idx}
                  className="group flex flex-col gap-4 animate-in fade-in slide-in-from-left-4 duration-500"
                  style={{ animationDelay: `${idx * 150}ms` }}
                >
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 font-korean" style={{ fontSize: `${fontSize}px` }}>
                    {line.words.map((word, wIdx) => (
                      <span
                        key={wIdx}
                        onClick={(e) => handleWordClick(e, word)}
                        className="text-white hover:text-brand-cyan hover:bg-brand-cyan/20 px-1 -mx-0.5 rounded transition-colors cursor-pointer font-korean"
                      >
                        {word.text}
                      </span>
                    ))}
                  </div>
                  <div className="text-[13px] font-medium text-white/60">{line.translation}</div>
                </div>
              ))}

              <div className="pt-12 mt-12 border-t border-zinc-800/50">
                <div className="flex items-center gap-2 text-brand-cyan font-bold uppercase tracking-[0.2em] text-[10px] mb-4">
          ...
          {selectedWord ? (
            <div className="p-5 lg:p-8 flex-1 min-h-0 overflow-y-auto scrollbar-hide animate-in slide-in-from-right-4 fade-in duration-300 flex flex-col">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="text-4xl md:text-5xl font-black text-white mb-2">{selectedWord.text}</h3>
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center p-8 opacity-40">
              ...
              <p className="text-zinc-400 font-medium max-w-sm text-lg">
                Кликните на любое слово в стихотворении слева, чтобы увидеть подробный разбор и перевод.
              </p>
            </div>
          )}
        </Panel>
